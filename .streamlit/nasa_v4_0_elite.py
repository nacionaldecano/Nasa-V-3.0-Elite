...
def get_option_expirations(ticker: str):
    """Obtiene expiraciones con reintentos."""
    import time
...
def load_option_chain(ticker: str, expiration: str):
    """Carga la cadena de opciones con reintentos y validacion robusta."""
    import time
    for attempt in range(3):
        try:
            tk = yf.Ticker(ticker)
            ...
            # Validar que tengan datos reales
            ...
            if "strike" not in calls.columns or "strike" not in puts.columns:
                time.sleep(1)
                continue
            if calls["strike"].dropna().empty or puts["strike"].dropna().empty:
                time.sleep(1)
                continue
            return calls, puts
        except Exception:
